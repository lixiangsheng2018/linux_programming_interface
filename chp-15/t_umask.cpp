...
int main(int argc, char *argv[])
{
    ...
    if (fd == -1)
        myc::errExit("open-%s", MYFILE);
    if (mkdir(MYDIR, DIR_PERMS) == -1)
    	myc::errExit("mkdir-%s", MYDIR);

    u = umask(0);               /* Retrieves (and clears) umask value */

    if (stat(MYFILE, &sb) == -1)
    	myc::errExit("stat-%s", MYFILE);
    printf("Requested file perms: %s\n", myc::filePermStr(FILE_PERMS, 0));
    printf("Process umask:        %s\n", myc::filePermStr(u, 0));
    printf("Actual file perms:    %s\n\n", myc::filePermStr(sb.st_mode, 0));

    if (stat(MYDIR, &sb) == -1)
    	myc::errExit("stat-%s", MYDIR);
    printf("Requested dir. perms: %s\n", myc::filePermStr(DIR_PERMS, 0));
    printf("Process umask:        %s\n", myc::filePermStr(u, 0));
    printf("Actual dir. perms:    %s\n", myc::filePermStr(sb.st_mode, 0));

    if (unlink(MYFILE) == -1)
    	myc::errMsg("unlink-%s", MYFILE);
    if (rmdir(MYDIR) == -1)
    	myc::errMsg("rmdir-%s", MYDIR);
    exit(EXIT_SUCCESS);
}
